...
class CrudCustomDemos extends DemoReadyEventEmitter(CrudDemo(PolymerElement)) {
  static get template() {
    return html`
    ...
    <p><i>NOTE: </i>That you need to add a <code>vaadin-crud-edit-column</code> in order to have a mechanism for editing the item</p>
      ...
      <template preserve-content="">
        <vaadin-crud exclude="_id, password">
          <vaadin-form-layout slot="form" style="max-width: 60em;">
            ...
            <br>
            <vaadin-text-field path="details.email" label="Email" required="" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\\.[a-z]{2,4}\$" error-message="invalid email"></vaadin-text-field>
            <vaadin-password-field path="password" label="Password" required="" pattern="(?=.*\\d)(?=.*[a-z])(?=.*[A-Z]).{6,}" error-message="min 6 upper/lower chars and digits"></vaadin-password-field>
            <br>
