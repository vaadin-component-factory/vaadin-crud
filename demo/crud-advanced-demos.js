...
class CrudAdvancedDemos extends DemoReadyEventEmitter(CrudDemo(PolymerElement)) {
  static get template() {
    return html`
    ...
    <p>For filtering and ordering it uses <code>vaadin-grid-filter</code> and <code>vaadin-grid-sorter</code> helpers
      that automatically add appropriate parameters to the <code>dataProvider</code> callback.</p>
    <vaadin-demo-snippet id="crud-advanced-demos-remote-data">
      <template preserve-content="">
        ...
        <vaadin-notification position="top-end">
          <template>Save and Delete operations are not implemented</template>
        </vaadin-notification>
        <script>
